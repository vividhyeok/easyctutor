...
> p(주소)와 *p(값)를 섮어서 쓴
> p+1을 "숫자 1 더하기"로 생각해버림
> 함수에서 배열 길이를 sizeof로 구하려고 함
> n을 잘못 넘겨서 범위를 벗어남 (i <= n 같은 실수)
> 
...
> 포인터 p를 써서 a[0]부터 a[4]까지 출력해보기 (*(p+i)로)
> 
...
> sum 변수를 선언만 하고 더하기를 돌려서(초기화 안 하고) 결과가 이상해지는지 직접 보기
> 전역 변수 g를 만들어서 함수가 몇 번 호출됐는지 세기(근데 어디서 바뻐는지 추적이 쉬운지/어려운지 체감하기)
> static 카운터 예제를 보고, static을 빼면 왜 매번 1이 나오는지 직접 확인하기
> 
...
> 이거 해보면 "아 이제 학생 데이터 관리가 되네" 느낌 바로 와요
> 배열로 훑는 건 똑같은데, 대상이 int가 아니라 "학생 한 덩어리"가 된 거
> 
...
> p는 Student의 주소
> p->score는 "p가 가리키는 학생의 score"
> 즉 . 대신 -> 쓰는 상황은 딱 하나: 포인터일 때
> 
