...
> - 문장 입력이 필요하면 `fgets` 중심으로 가요
> - 숫자도 문자열로 받고 `atoi` 같은 걸로 바꾸는 방식도 있어요
> 
...
> - 입력을 문자열로 받으면 “공백/엔터 문제”를 통제하기 쉬워져요
> 
...
> - 여러 정보를 묶어서 저장하고 싶다
> - 사람 정보(이름/나이), 학생 정보(학번/점수)처럼 "세트 데이터"를 다루고 싶다
> 
...
> - 학생 한 명 = 학번 + 점수 + 이름
> - 근데 배열로 하면 `id[]`, `score[]`, `name[][]` 이렇게 갈라져요
> - 그리고 이런 식이면 같은 학생 정보가 서로 다른 배열에 흩어져서 관리가 힘들어져요
> - 무엇보다 무서운 건... 인덱스 한 번 삐끗하면 조용히 틀린 값이 연결돼요 (이게 제일 화나요)
> 
...
> - `struct Student`는 "학생 한 명은 이런 정보로 구성된다"라는 틀(설계도)예요
> 
...
> - `s.id` : s라는 학생의 id
> - `s.score` : s라는 학생의 score
> - `.`는 그냥 "안으로 들어가서 그 멤버를 고른다" 느낌이에요
> 
...
> - `scanf`는 "값"을 받는 게 아니라 **저장할 위치(주소)**가 필요해서예요
> - 정수 멤버는 `&` 붙는 게 정상이에요
> 
...
> - 문자열(배열)은 대체로 `&`를 안 붙이는 패턴이 많아요
> - 지금 단계에서는 "아 문자열은 그냥 이렇게 받는구나"로 넘어가도 돼요
> 
...
> - typedef는 그냥 "긴 타입 이름을 짧게 별명 붙여주는 것"이에요
> - 입문 단계에서는 이 패턴 하나만 알아도 충분해요
> 
...
> - 배열로 훑는 건 똑같은데, 대상이 int가 아니라 "학생 한 덩어리"가 된 거예요
> 
...
> - `p`는 Student의 주소예요
> - `p->score`는 "p가 가리키는 학생의 score"예요
> - 즉 `.` 대신 `->` 쓰는 상황은 딱 하나: 포인터일 때예요
> 
...
참고로 이건 괄호 때문에 더 복잡해서 잘 안 써요.

> - 근데 이건 괄호 때문에 초보 때 보기 힘드니까
> - 그냥 **포인터 구조체는 ->**로 읽는 게 제일 편해요
> 
...
> - 값 전달: 함수 안에서 바꿔도 원본은 그대로일 수 있어요
> - 주소 전달: 원본을 직접 만져요
> 
...
> - scanf("%d", s.id); 처럼 & 빼먹기
> - a[i].id 해야 하는데 a.id처럼 인덱스 놓치기
> - 포인터인데 p.score라고 쓰기 (포인터면 p->score)
> - 구조체 여러 개를 "배열 여러 개로 쪼개서" 다시 관리하려는 습관 (구조체 쓸 이유가 사라짐)
> 
...
> - 최댓값 찾기 패턴은 8장에서 하던 거랑 똑같아요
> - 단지 비교 대상이 a[i]의 score가 된 것뿐이에요
> - "학생 한 명 단위로 처리한다"가 구조체의 핵심 이득이에요
> 
...
- 학생 5명의 (학번, 점수)를 입력받아서 평균 점수 출력하기
- 최고 점수 학생의 학번만 출력하기
- 점수가 60 미만인 학생 수 세기
