...
입력에 `minhyuk` 같은 걸 넣으면

> Hello, minhyuk! 이런 식으로 나올 거예요.
> 
...
다음 장에서는 문자열을 “다루는 기본 도구”를 잡을 거예요.
...
## 10-4. `strlen`을 직접 만들면 이런 느낌이에요

이거 처음 보면 "뭐야 이게 무슨 소리야" 싶을 수 있는데

괜찮아요.

지금은 "아 이런 식으로 만들어지는구나" 정도만 보고 넘어가도 돼요.
...
## 10-6. `strcpy`를 직접 만들면 이런 느낌이에요

복사는 결국 한 글자씩 옮기는 거예요.

그리고 마지막 `\0`도 같이 옮겨야 끝나요.
...
> 단어 2개를 입력받아서, 더 긴 단어가 뭐지 출력하기
> 단어 2개가 같으면 "OK", 다르면 "NO" 출력하기
> 단어 1개를 입력받아서, 그 단어를 다른 배열에 복사해서 출력하기
> 
...
## 11-1. 문자열 입력이 갑자기 어려워지는 이유

지금까지는 `scanf("%d")` 같은 숫자 입력이라 편했어요.

근데 문자열은 바로 이런 문제가 생겨요.

> 공백이 들어가면 중간에서 끊긴다
> 입력 끝에 \n(엔터)이 남는다
> 배열 크기보다 길게 들어오면 위험하다
> 

그래서 11장은 “문자열 입력에서 덜 터지게” 만드는 장이에요.

---

## 11-2. `scanf("%s")`는 “단어”만 받는다고 생각해요
...
## 11-3. 공백까지 포함해서 한 줄을 받고 싶으면 `fgets`

문장(한 줄)을 받는 대표적인 방법이 `fgets`예요.
...
그래서 출력할 때 줄이 한 번 더 내려가거나,

문자열 비교할 때 갑자기 안 맞을 수 있어요.
...
처음엔 "왜 이렇게 복잡해?" 싶을 수 있는데

이 패턴 하나만 외워두면 나중에 계속 써먹을 수 있어요.

---

## 11-6. 숫자 입력 다음에 `fgets`가 “빈 줄”로 들어오는 이유
...
> 빈 줄이 들어온 것처럼 보이는 거예요
> 

---

## 11-7. 이 문제를 피하는 가장 쉬운 방법 (입문용)

입문 단계에서는 “입력 스타일을 섞지 않는 것”이 제일 편해요.

> 문장 입력이 필요하면 fgets 중심으로 간다
> 숫자도 문자열로 받고 atoi 같은 걸로 바꾸는 방식도 있다
> 

근데 지금 단계에서는 너무 많이 확장하지 말고,

일단은 “섞으면 위험할 수 있다”만 잡고 가요.

---

## 11-8. `fgets`로 숫자도 받고 정수로 바꾸는 방법 (맛보기)

문자열로 한 줄 받고, 숫자로 바꾸는 방식이에요.
...
> 입력을 문자열로 받으면 “공백/엔터 문제”를 통제하기 쉬워진다
> 

---

## 11-9. 직접 쳐볼 예시: 문장 한 줄 받고 그대로 출력하기
...
> 한 줄을 입력받아서 길이를 출력하기 (\n 제거 후 길이)
> 두 줄을 입력받아서, 두 문장이 같으면 "SAME", 다르면 "DIFF" 출력하기
> 한 줄을 입력받아서, 그 안에 공백이 몇 개인지 세어보기 (반복문 + 배열 느낌)
> 
...
> add(3, 5)는 "계산해서 값 하나로 바뻐다"
> 그래서 변수에 넣을 수도 있고, 바로 출력할 수도 있다
> 
...
> 배열은 통째로 "값 복사"가 아니라 "묶음 전달"처럼 넘어간다
> 그래서 길이 n을 같이 넘겨주는 게 보통이다
> 
...
> 함수 안에서 x를 바뻐는데, main의 x는 그대로다
> 배열은 함수에서 바뻐면 main에서도 바뻐다
> scanf는 왜 변수 앞에 &를 붙이라고 하지?
> 
...
> addOne(a)는 “a를 복사해서 x에 넣는다”함수 안에서 바뀐 건 xa는 그대로
> 
...
> addOne(&a) : a의 주소를 넘김*p = *p + 1 : 주소가 가리키는 “원본 값”을 바꿈
> 
...
> p : 주소*p : 그 주소에 있는 값(원본 값)
> 

그래서 위 코드는 말로 하면 이거예요.

> a의 주소를 받아서그 위치의 값을 1 증가시켜라
> 
...
> 배열은 값 복사가 아니라“원본을 가리키는 것”이 넘어간다
> 
...
> scanf("%d", x);처럼 &를 빼먹기포인터 변수에 아무 주소도 안 넣고 *p를 쓰기 (위험)*p랑 p를 섞어서 읽기“값 전달이 기본”이라는 걸 잊고 함수가 바깥 값을 바꿀 거라 착각하기
> 
...
> swap을 직접 다시 작성해서 동작 확인하기addTwo(int *p)를 만들어서 원본 값을 2 증가시키기setZero(int a[], int n)를 만들어서 배열을 전부 0으로 만들기
> 
...
> 주소를 쓰기 시작했다그러면 “포인터 문법”을 제대로 한 번 정리해야 한다
> 
...
> 포인터는 “주소를 저장하는 변수”&는 “주소를 꺼내는 기호”*는 “그 주소의 값을 꺼내는 기호”
> 
...
> a랑 *p는 결국 같은 값을 본다이유는 p가 a의 주소를 들고 있기 때문
> 
...
> p에 주소를 넣기 전에 *p를 써버림&를 안 붙여서 값이 넘어가 버림 (scanf, swap에서 자주)p와 *p를 섞어서 읽음포인터 여러 개를 한 줄에 선언하다가 한 개만 포인터가 됨
> 
...
> int a = 10; int *p = &a;로 시작해서 *p를 바꿔보고 a가 바뀌는지 확인하기addOne(int *p) 함수를 만들고 main에서 &a를 넘겨서 실제로 바뀌는지 확인하기swap을 다시 써보고, &를 빼면 왜 안 되는지 직접 에러/결과로 확인하기
> 
...
> 포인터 문법을 배웠다근데 배열이랑 만나는 순간, 포인터가 “갑자기 자주 보인다”
> 
...
완전히 “항상 같다”라고 말하면 예외가 있어서 복잡해지는데,

입문 단계에서는 이렇게 생각해도 충분해요.
...
> int *p면 p+1은 “다음 int 칸”char *p면 p+1은 “다음 char 칸”
> 
...
> *p는 첫 칸*(p+1)은 두 번째 칸p[2]도 결국 같은 방식
> 
...
배열을 함수로 넘길 때는 보통 두 개를 같이 넘겨요.

> 배열 시작 주소길이
> 

왜냐면 배열은 함수로 넘어갈 때 “크기 정보”가 같이 안 넘어오는 경우가 많아요.
...
> sizeof로 길이 구하는 건 “배열을 직접 들고 있을 때만” 믿자함수로 넘길 땐 길이를 같이 넘기자
> 
...
> sum(a, n)에서 a는 시작 주소처럼 넘어간다그래서 함수가 배열을 “쭉 훑을 수 있다”
> 
...
> p(주소)와 *p(값)를 섞어서 씀p+1을 “숫자 1 더하기”로 생각해버림함수에서 배열 길이를 sizeof로 구하려고 함n을 잘못 넘겨서 범위를 벗어남 (i <= n 같은 실수)
> 
...
> a[i]와 *(a+i)가 같은지 직접 출력해보기배열을 함수에 넘겨서 최댓값을 찾는 max(int *a, int n) 만들기포인터 p를 써서 a[0]부터 a[4]까지 출력해보기 (*(p+i)로)
> 
...
> 배열은 첫 칸 주소처럼 동작한다그럼 “문자열”은 뭐냐?사실 문자열은 char 배열이다
> 
...
> 문자열 배열 이름 s 자체가 시작 주소처럼 동작해서 &를 안 붙이는 경우가 많다
> 

그리고 더 중요한 조심 포인트가 있어요.

> %s 입력은 공백에서 끊긴다길이 제한 없이 받으면 위험할 수 있다
> 

입문 단계에서는 일단 이렇게만 기억해도 돼요.

> 공백 없는 한 단어 입력이면 %s로 충분문장 입력은 다음 장에서 더 안전한 방법으로
> 
...
> 문자열은 \0까지 세면 길이가 나온다결국 반복문 + 배열이다
> 
...
> 앞에서부터 한 글자씩 비교다르면 거기서 끝끝까지 같으면 같은 문자열
> 
...
> 배열 크기를 너무 작게 잡음 (\0 자리 없음)'\0'을 빼먹고 복사/출력해서 이상한 값이 나옴문자열을 ==로 비교하려고 함입력이 길어지면 배열 범위를 넘을 수 있다는 걸 잊음
> 
...
> 입력받은 문자열 길이를 직접 세서 출력하기 (my_strlen)문자열 두 개를 입력받아서 같은지 판별하기 (my_strcmp == 0)문자열을 하나 입력받아서 “역순으로 출력”해보기 (배열 인덱스 역방향 연습)
> 
...
> 글자 여러 개를 저장하고반복문으로 처리하고직접 함수처럼 만들어서 쓸 수 있다
> 

그래서 다음 장은 딱 자연스러워요.

> 함수를 직접 만들고 잘 쓰는 법“입력 → 처리 → 출력”을 함수로 분리하는 감각
> 
...
> 같은 패턴이 계속 반복된다한 번 고치면 여기저기 다 고쳐야 한다main()이 너무 길어서 뭐 하는지 안 보인다
> 

그래서 함수를 써요.

> 함수는 “코드 덩어리”에 이름을 붙이는 거예요그리고 필요할 때 불러서 실행하는 거예요
> 
...
> 반환형: 결과가 뭐냐 (int, double, void 등)함수이름: 부를 이름매개변수: 함수가 사용할 재료(입력)return: 결과를 돌려주기
> 
...
> 함수는 “계산을 대신 해주는 부품”이다add(3, 5)는 결과값 하나로 바뀐다
> 
...
> return은 값을 돌려주는 동시에, 그 자리에서 함수 종료
> 
...
> void는 “돌려줄 값 없음”대신 함수 내부에서 출력/처리만 하고 끝난다
> 
...
> 같은 코드가 2번 이상 나온다 → 함수 후보main()이 너무 길다 → 큰 덩어리를 함수로 나누기“이 부분이 뭔지” 이름만 봐도 알게 하고 싶다 → 함수로 빼기
> 
...
> 함수는 “입력(배열, 길이)”을 받아서“결과(합)”를 돌려준다
> 
...
> 배열 전체가 복사되는 게 아니다시작 위치(첫 칸)를 넘기는 느낌이다그래서 보통 길이 n을 같이 넘긴다
> 
...
> 함수 선언/정의 위치 때문에 컴파일 에러매개변수 개수/타입이 안 맞아서 이상한 값return 타입이랑 실제 반환값이 안 맞음void인데 값을 return 하려 함 (또는 그 반대)
> 

초반에는 이렇게만 기억해도 돼요.

> 함수는 “입력 타입”과 “출력 타입”이 정확해야 한다
> 
...
문자열 길이 구하는 my_strlen을 함수로 만들고 재사용하기
...
> 변수가 어디까지 살아있냐(범위)함수 안/밖에서 변수가 어떻게 다르냐
> 

그래서 다음 장에서는

> 지역 변수 / 전역 변수그리고 “변수 범위” 감각
> 
...
> main에 있는 변수는 함수에서 못 쓰네?함수 안에서 만든 변수는 함수 끝나면 사라지네?이름이 같은 변수를 또 만들면 뭐가 우선이지?전역 변수는 왜 쓰지 말라 그러는지 감이 안 오네?
> 
...
> 변수가 “선언된 위치” 기준으로어디까지 접근 가능한지(보이는지)가 정해져요그걸 범위(scope) 라고 불러요
> 
...
> { } 안에서 선언한 변수는그 { } 안에서만 살아 있어요밖으로 나오면 못 써요
> 
...
> x는 if 블록 안에서만 존재블록 밖에서는 “없는 변수” 취급
> 
...
> 지역 변수는 함수 안에서만 보임함수가 끝나면 같이 사라짐
> 
...
> 안쪽 블록의 x가 바깥 x를 잠깐 가림블록을 벗어나면 바깥 x가 다시 보임
> 
...
> 전역 변수는 “모든 함수에서 접근 가능”대신 너무 쉽게 여기저기서 바뀌어서코드가 커질수록 추적이 어려워져요
> 
...
> 어디서 값이 바뀌었는지 추적이 안 됨함수가 “입력 없이도 결과가 바뀌는” 이상한 모양이 됨디버깅이 갑자기 어려워짐
> 

그래서 초반 기준은 이거로 가면 편해요.

> 웬만하면 지역 변수 + 매개변수로 전달전역은 “진짜 공통 상태”에만 (그리고 최소화)
> 
...
> 변수를 만들면 바로 초기화하기특히 sum, cnt, mx 같은 누적 변수는 무조건
> 
...
> 함수를 몇 번 호출했는지 세고 싶다이전 호출 결과를 기억하고 싶다
> 
...
> static이면 함수가 끝나도 값이 유지됨일반 지역 변수면 호출마다 새로 만들어짐
> 
...
> { } 안에서 만든 변수는 { } 안에서만 쓴다함수 안에서 만든 변수는 그 함수 안에서만 쓴다 (지역 변수)함수 바깥에서 만든 변수는 어디서든 보인다 (전역 변수)이름이 겹치면 “가까운 변수”가 우선한다지역 변수는 초기화 습관이 중요하다static은 “값이 남는 지역 변수”다(맛보기)
> 
...
> sum 변수를 선언만 하고 더하기를 돌려서(초기화 안 하고) 결과가 이상해지는지 직접 보기전역 변수 g를 만들어서 함수가 몇 번 호출됐는지 세기(근데 어디서 바뀌는지 추적이 쉬운지/어려운지 체감하기)static 카운터 예제를 보고, static을 빼면 왜 매번 1이 나오는지 직접 확인하기
> 
...
> int만으로는 표현하기 힘든 “한 덩어리 데이터”를묶어서 다루는 방법예: 학생(이름, 학번, 점수), 좌표(x, y), 날짜(년/월/일)
> 
...
> 학생 1명의 정보가 있다이름(문자열)학번(정수)점수(정수)이걸 따로따로 변수로 들고 있으면 너무 흩어져요배열로도 애매해요 (자료형이 다 다르니까)
> 

그래서 필요한 게 구조체예요.

> 서로 다른 자료형을하나의 “묶음”으로 만들기
> 
...
> 내가 필요한 데이터 묶음을 직접 정의한다그 묶음을 변수처럼 쓴다
> 
...
> Student는 “학생 데이터 묶음”의 설계도실제 값은 아직 없음이 설계도로 변수를 만들어야 진짜로 쓸 수 있음
> 
...
> 구조체 변수 s의 내부 칸을 찍어서 접근한다배열 인덱스랑 비슷하게 “칸을 고른다”는 감각
> 
...
> a[i]는 i번째 학생(구조체 하나)그 안의 멤버는 .id, .score로 접근즉 a[i].score 같은 형태가 기본
> 
...
> 구조체를 값으로 넘기면 “복사”될 수 있다바깥 값을 바꾸려면 “주소”를 넘기는 게 편하다
> 
...
> 구조체를 함수에서 바꾸고 싶으면 주소를 넘긴다->는 “구조체 포인터 전용 점(.)” 같은 느낌
> 
...
> s.name은 문자열 배열이라 & 안 붙는 경우가 많다s.id, s.score는 int라서 &가 붙는다
> 
...
> 구조체 멤버에 접근할 때 . / ->를 섞어 씀구조체 배열에서 a[i].score 대신 a.score[i] 같은 식으로 헷갈림scanf에서 & 붙여야 할 곳과 아닌 곳을 반대로 함구조체 포인터인데 p.id처럼 점(.)을 써버림 (이때는 p->id)
> 
...
> 학생 3명을 입력받아서, 점수 평균을 출력하기 (구조체 배열 + 반복문)최고 점수 학생의 이름을 출력하기 (max 패턴을 구조체에 적용)setScore(struct Student *p, int score) 같은 함수를 하나 더 만들어서 구조체 값 바꾸기 연습하기
> 
...
> 구조체를 함수로 넘길 때 “복사 vs 주소” 감각 더 잡기여러 데이터를 만들고 지우고 싶을 때 필요한 것(동적 메모리의 맛보기)그리고 C에서 자주 쓰는 “typedef”로 구조체를 더 편하게 쓰는 방법
> 
...
> malloc : 메모리 빌리기 (새로 만들기)free : 메모리 반납하기 (다 쓰면 지우기)
> 
...
> sizeof(int) * 5 만큼의 공간을 빌린다결과는 주소라서 int *a에 담는다배열처럼 a[i]로 쓴다마지막에 free(a)로 반납한다
> 
...
> sizeof(int) : int 한 칸 크기sizeof(double) : double 한 칸 크기“칸 크기 × 칸 개수” 만큼 빌린다
> 
...
> n을 먼저 입력받고n칸을 빌리고배열 문제 패턴(입력→처리→출력)을 그대로 쓴다
> 
...
> 기존 포인터를 바로 덮어쓰지 말고, 새 포인터로 받아서 성공 확인실패하면 NULL이 나올 수 있다
> 
...
> malloc 했는데 free 안 해서 메모리 누수free 한 다음에 또 쓰는 실수 (반납 후 사용)free를 두 번 하는 실수 (double free)malloc 결과가 NULL인데도 그대로 쓰는 실수“칸 개수”가 아니라 “바이트”를 빌려야 한다는 걸 잊는 실수malloc(n) 이 아니라 malloc(sizeof(int)*n)
> 
...
> n을 입력받고 정수 n개를 동적 배열에 저장한 뒤, 최솟값 출력하기n을 입력받고 정수 n개를 저장한 뒤, 짝수의 개수 출력하기n을 입력받고 정수 n개를 저장한 뒤, 거꾸로 출력하기
> 
...
> “데이터를 메모리에만 두지 말고 파일로 저장하기”“저장한 걸 다시 읽어오기”
> 
...
> 실행할 때 입력하고화면에 출력하고프로그램이 끝나면 다 사라짐
> 

근데 현실에서는 이런 걸 하고 싶어져요.

> 결과를 파일로 저장하고 싶다다음에 다시 실행해도 데이터를 그대로 쓰고 싶다입력을 매번 치기 귀찮다 (파일에서 읽고 싶다)
> 
...
> 파일 열기 (fopen)읽기/쓰기 (fprintf, fscanf, fgets, fputs 등)파일 닫기 (fclose)
> 
...
> 그게 FILE *보통 이름은 fp로 씀NULL이면 열기 실패
> 
...
> "r" : 읽기 모드 (read)"w" : 쓰기 모드 (write)파일이 없으면 새로 만들고파일이 있으면 내용이 덮어써짐
> 
...
> 쓰기 버퍼가 남아있으면 반영이 안 될 수 있음파일 자원을 반납해야 함
> 
...
> 파일을 "r"로 열었는데 파일이 없어서 NULL"w"로 열었는데 기존 내용이 날아감(덮어쓰기)fopen 실패 체크 안 해서 fp가 NULL인데 읽고/쓰기 시도fclose를 안 해서 저장이 덜 되거나 자원 누수fscanf가 실패했는데도 계속 읽는다고 가정함
> 
...
> fscanf가 1을 반환하면 정수 1개 읽기 성공실패하면 반복 종료
> 
...
> 정수 n개를 입력받아 nums.txt에 저장하기 (한 줄에 하나씩)nums.txt를 다시 열어서 모든 숫자의 합 출력하기nums.txt를 읽어서 최댓값 출력하기 (while + fscanf 패턴 써보기)
> 
...
그 다음에 자연스럽게 나오는 건 이런 거예요.

> 여러 정보를 묶어서 저장하고 싶다사람 정보(이름/나이), 학생 정보(학번/점수)처럼 “세트 데이터”를 다루고 싶다
> 

그래서 다음 장은 보통 **구조체(struct)** 로 넘어가요.

---

# 22장. 구조체 — “데이터 한 덩어리”를 만드는 법

## 22-1. 구조체는 왜 나오냐면… 배열만으로는 슬슬 한계가 와요

8장까지는 “정수 여러 개” 같은 걸 배열로 잘 다뤘죠.

근데 현실 데이터는 보통 이렇게 생겨요.

> 학생 한 명 = 학번 + 점수 + 이름근데 배열로 하면 id[], score[], name[][] 이렇게 갈라짐그리고 이런 식이면 같은 학생 정보가 서로 다른 배열에 흩어져서 관리가 힘들어져요무엇보다 무서운 건… 인덱스 한 번 삐끗하면 조용히 틀린 값이 연결됨 (이게 제일 빡침)
> 

그래서 C는 “이것들을 **한 세트로 묶는 문법**”을 줘요. 그게 구조체.

---

## 22-2. 구조체는 “설계도”부터 만든다고 생각하면 편해요

구조체는 일단 “이런 구성으로 묶을 거야”라는 설계도를 만들어요.
...
> struct Student는 “학생 한 명은 이런 정보로 구성된다”라는 틀(설계도)
> 

아직 학생 한 명이 만들어진 게 아니라, “틀”만 만든 거예요.

---

## 22-3. 설계도 만들었으면 이제 “진짜 학생” 하나 만들어봐요
...
> s.id : s라는 학생의 ids.score : s라는 학생의 score.는 그냥 “안으로 들어가서 그 멤버를 고른다” 느낌
> 

---

## 22-4. 입력 받을 때: 여기 진짜 많이 터져요 (scanf랑 구조체)
...
> scanf는 “값”을 받는 게 아니라 **저장할 위치(주소)**가 필요해서정수 멤버는 & 붙는 게 정상이에요
> 
...
> 문자열(배열)은 대체로 &를 안 붙이는 패턴이 많아요지금 단계에서는 “아 문자열은 그냥 이렇게 받는구나”로 넘어가도 됨
> 

---

## 22-5. `typedef` 쓰면 구조체가 갑자기 ‘사람 친화적’이 돼요

솔직히 `struct Student s;` 계속 쓰기 귀찮죠.
...
> typedef는 그냥 “긴 타입 이름을 짧게 별명 붙여주는 것”입문 단계에서는 이 패턴 하나만 알아도 충분해요
> 

---

## 22-6. 구조체 배열: 여기서 갑자기 “진짜 프로그램” 느낌 나요
...
> a[i] : i번째 학생(한 덩어리)a[i].score : i번째 학생의 점수a[i].id : i번째 학생의 학번
> 
...
> 이거 해보면 “아 이제 학생 데이터 관리가 되네” 느낌 바로 와요배열로 훑는 건 똑같은데, 대상이 int가 아니라 “학생 한 덩어리”가 된 거
> 

---

## 22-7. 구조체 포인터 나오면 `>` 이거 하나만 추가라고 생각해요

구조체를 함수에서 바꾸고 싶으면 포인터로 넘기죠?
...
void add10(Student *p) {
    p->score += 10;
}

```

> p는 Student의 주소p->score는 “p가 가리키는 학생의 score”즉 . 대신 -> 쓰는 상황은 딱 하나: 포인터일 때
> 
...
> 근데 이건 괄호 때문에 초보 때 보기 빡세니까그냥 **포인터 구조체는 ->**로 읽는 게 제일 편함
> 

---

## 22-8. 함수에 구조체 넘길 때, “복사냐 원본이냐”가 갈려요

이거 13장(값/주소 전달)에서 했던 그 감각 그대로예요.
...
```c
void setZero2(Student *p) {
    p->score = 0;
}

```

> 값 전달: 함수 안에서 바꿔도 원본은 그대로일 수 있음주소 전달: 원본을 직접 만짐
> 

---

## 22-9. 구조체에서 초보가 자주 터지는 실수 모음 (여기만 잘 피해도 됨)

> scanf("%d", s.id); 처럼 & 빼먹기a[i].id 해야 하는데 a.id처럼 인덱스 놓치기포인터인데 p.score라고 쓰기 (포인터면 p->score)구조체 여러 개를 “배열 여러 개로 쪼개서” 다시 관리하려는 습관 (구조체 쓸 이유가 사라짐)
> 

---

## 22-10. 직접 쳐볼 예시: 최고 점수 학생 찾기 (구조체 맛이 제일 잘 남)
...
int main(void) {
    ...
    printf("%d %d\n", a[best].id, a[best].score);
    return 0;
}

```

> 최댓값 찾기 패턴은 8장에서 하던 거랑 똑같아요단지 비교 대상이 a[i]의 score가 된 것뿐“학생 한 명 단위로 처리한다”가 구조체의 핵심 이득임
> 
...
> 학생 5명의 (학번, 점수)를 입력받아서 평균 점수 출력하기최고 점수 학생의 학번만 출력하기점수가 60 미만인 학생 수 세기
> 
...
# 23장. 마무리 — 이제 “C가 무섭지 않은 상태”가 됐어요

## 23-1. 여기까지 왔으면, 사실 이미 게임 끝났어요

처음엔 진짜 다 이렇게 느껴져요.

> ; 하나 빠져도 에러괄호 어디 닫는지도 모르겠고scanf에 & 붙이라니까 그냥 외웠고배열 나오면서 “이거부터 어려운데?” 싶고문자열은 \0 때문에 갑자기 이상한 세계 같고포인터는 기호가 너무 많아서 일단 무서움
> 

근데 지금은 달라요.

> 코드가 “왜 이렇게 생겼는지” 대충 감이 있고에러가 나도 “어디부터 봐야 하는지” 감이 있고무엇보다, 대충이 아니라 직접 만들어본 게 있다
> 
...
## 23-2. 이 웹사이트 기준으로 “기초”는 어디까지냐?

이 흐름대로 왔다면, 기초는 사실 여기서 끝났다고 봐도 돼요.

> 변수 / 연산 / 입출력조건문 / 반복문배열 + 배열로 문제 풀기문자열(= char 배열 + \0)함수(코드 쪼개기)값/주소 전달(왜 어떤 건 바뀌고 어떤 건 안 바뀌는지)포인터(주소 문법)배열-포인터 관계(배열이 왜 주소처럼 보이는지)구조체(데이터를 한 덩어리로 묶기)
> 
...
## 23-3. 근데 여기서 갑자기 실력이 늘지 않는 느낌이 들 수 있어요

여기까지 공부하면 이런 현상이 와요.

> “내용은 이해한 것 같은데…”“막상 문제 보니까 손이 안 움직이는데?”“내가 진짜 안다고 말할 수 있나?”
> 
...
> 문제를 읽고필요한 변수를 잡고반복문/조건을 조합해서내 머리로 설계하는 단계
> 
...
## 23-4. 그래서 내가 추천하는 “마무리 루틴”이 있어요

여기서부터는 “한 번 더”가 진짜 중요해요.

> 이 사이트를 처음부터 2~3번 더 훑어보세요대신 처음처럼 꼼꼼히 보지 말고, 빠르게 훑는 느낌으로“아 이거 이때 나왔지” 감각만 남겨도 충분해요두 번째부터는 특히 코드만 보고도 무슨 장인지 떠올리는 상태가 목표예요
> 

그리고 진짜 중요한 거.

> 반드시 알고리즘 문제도 풀어보시는 것을 추천드릴게요!이건 “공부를 확인하는 용도”가 아니라C가 몸에 붙는 과정이라서 그래요
> 

문제는 처음엔 쉬운 걸로 가면 돼요.

> 배열 합/최댓값/최솟값/개수문자열 길이/공백 개수/대소문자 변환구조체로 학생 정보 정렬 “비슷한 거” (정렬 자체는 다음 단계지만, 비교 기준 잡는 감각이 중요)
> 

---

## 23-5. 다음에 뭘 하면 “C가 더 강해지냐?”

여기서부터는 선택지가 몇 개 있어요.

> 파일 입출력 (fopen, fprintf, fscanf)동적 메모리 (malloc, free)구조체 + 포인터 조합 (연결 리스트 같은 자료구조로 이어짐)정렬/탐색 같은 알고리즘 패턴 (진짜 실력 구간)
> 

근데 솔직히 말하면,

> 지금 당장 뭘 더 배우기 전에배열/문자열/함수 문제를 조금만 더 풀면그 다음 단계가 “안 무섭게” 들어가져요
> 

---

## 23-6. 마지막으로, 진짜로 수고했어요

여기까지 왔다는 건요,

> 그냥 “설명 들은 사람”이 아니라손으로 한 번이라도 만들어본 사람이라는 뜻이에요
> 
...
> 다시 한 번 말하지만처음부터 몇 번 더 보는 걸 추천드리고,반드시 알고리즘 문제도 풀어보시는 것을 추천드릴게요!
> 
